...
2. props 适用于：
    ...
    (2).子组件 ==> 父组件 通信（要求父先给子一个函数），把函数传输里面，从里面把数据取出，像是一个间谍一样
...
3. 使用方式：
   ...
   3. 作用域插槽：
      ...
      1. 理解：<span style="color:red">数据在子组件的自身，但根据数据生成的结构需要组件的使用者来决定。也就是说，父组件在使用子组件的时候传递html结构，但是里面的数据却是使用子组件的。</span>（games 数据在 Category 组件中，但使用数据所遍历出来的结构由 App 组件决定）
   ...
   ```
插槽的本质是父子组件之间的关系，插槽让父组件可以输入html结构到子组件里面去，插槽的位置是在子组件里面，他的目的是让父组件的内容来填充，
   ```
...
2. 具体编码：
   ...
   ```vue
   <keep-alive include="News">  //  News是指组件名字
       <router-view></router-view>
   </keep-alive>
   ```
